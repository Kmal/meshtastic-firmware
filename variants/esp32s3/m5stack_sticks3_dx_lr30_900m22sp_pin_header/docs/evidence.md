...
- Corrected invariant: DX VCC is StickS3 Hat2 pin 13 (`3V3_L2`) only, and early init must set M5PM1 `PWR_CFG` bit 2 (`LDO_EN`) so that rail is enabled. StickS3 Hat2 `EXT_5V`, `5V_IN`, and `BAT` are not valid DX VCC sources for this target.
...
- Corrected code invariant: early init enables the M5PM1 3.3 V LDO bit for `3V3_L2`, and `SX126X_MAX_POWER` is explicitly set to 22 dBm, matching the DX-LR30-900M22S maximum transmit-power spec.
...
- StickS3 hardware evidence identifies internal I2C as GPIO47/GPIO48, M5PM1 `LDO_EN` as the 3V3_L2/Hat2 radio-power enable,
